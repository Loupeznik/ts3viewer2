...
export const MainPage = () => {
    ...
    function renderServerInfo(info: ServerInfoDto) {
        let clientsOnline = (info.clientsOnline as number) - (info.queriesOnline as number)
        return (
            <div className="text-center">
                <div className="mb-1 text-md font-semibold">Currently connected users</div>
                <div className="my-2 font-sans text-xl font-bold">{!clientsOnline ? "0" : clientsOnline}/{!info.maxClients ? "0" : info.maxClients }</div>
            </div>
        )
    }
